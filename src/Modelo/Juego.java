...
public class Juego {
    ...
    /** NUEVO: recibe modoAutomatico para decidir si marca los cartones */
    public int procesarSiguienteNumero(boolean modoAutomatico) {
        ...
        if (modoAutomatico) marcarCartones(numero);
    ...
    public List<Point> obtenerLineaGanadora(Carton carton) {
        ...
        if (modoJuego == ModoJuego.CUATROESQUINAS) {
            if (c[0][0].isMarcados() && c[0][4].isMarcados() &&
                c[4][0].isMarcados() && c[4][4].isMarcados()) {
